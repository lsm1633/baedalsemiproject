	...
	<div class="container">
		...
		<div class="bodyFrame col-sm-10"
			...
			<div class="body-title">
				<h3>
					<span class="glyphicon glyphicon-question-sign"></span>질문과 답변
				</h3>
			</div>
			<div id=formbody>
				<div id=subject>
				...
				<div id=shoulder>
					<%
						/* 작성자, 날짜 및 시간 */
					%>
					<div id=shoulder_p1>이름: ${dto.userName}</div>
					<div id=shoulder_p2>${dto.created}</div>
				</div>
			...
			<c:if test="${sessionScope.member.userId=='admin'}">
				...
				<div id=ankle_p1>
					...
					<c:if test="${not empty preReadDto }">
						<a
							href="<%=cp%>/qna/article.do?${params}&boardNum=${preReadDto.boardNum}">${preReadDto.subject}</a>
					</c:if>
				</div>
				...
				<div id=ankle_p2>
					...
					<c:if test="${not empty nextReadDto }">
						<a
							href="<%=cp%>/qna/article.do?${params}&boardNum=${nextReadDto.boardNum}">${nextReadDto.subject}</a>
					</c:if>
				</div>
			</c:if>
				<div id=footer>
					...
					<div id=footer_p1>
						...
						<c:if test="${sessionScope.member.userId=='admin'}">
							<c:if test="${dto.depth==0}">
								<input type="submit" name="btn"
									class="btn btn-default btn-sm wbtn" value=" 답글 "
						...
						<c:if test="${sessionScope.member.userId==dto.userId}">
							<input type="submit" name="btn"
								class="btn btn-default btn-sm wbtn" value=" 수정 "
								onclick="updateBoard();">
						</c:if>
						...
						<c:if
							...
							<input type="submit" name="btn"
								class="btn btn-default btn-sm wbtn" value=" 삭제 "
								onclick="deleteBoard();">
						</c:if>
					</div>
					<div id=footer_p2>
						<input type="submit" name="btn"
							class="btn btn-default btn-sm wbtn" value=" 목록 "
