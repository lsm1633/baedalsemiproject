...
public class FreeBoardServlet extends MyServlet {
	...
	protected void process(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (uri.indexOf("freeboard.do") != -1) {
			...
			// 이전글 다음글
			FreeBoardDTO preReadDto = dao.preReadBoard(dto.getGroupNum(), dto.getOrderNo(), searchKey, searchValue);
			FreeBoardDTO nextReadDto = dao.nextReadBoard(dto.getGroupNum(), dto.getOrderNo(), searchKey, searchValue);
			...
			dataCount = dao.dataCountReply(boardNum);
			...
			// 리스트에 출력할 데이터
			List<ReplyDTO> list = dao.listReply(boardNum, start, end);
			...
			if (info == null) { // 로그인되지 않은 경우
				...
				rdto.setUserName(info.getUserName());
